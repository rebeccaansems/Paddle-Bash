...
public class GameController : MonoBehaviour
{
    ...
    {
        string currScore = "";
        foreach (PlayerData players in SessionData.k_Players.Where(x => x != null))
        {
            currScore += players.Score + " | ";
        }
        CurrentScore.text = currScore.TrimEnd('|', ' ');

        for (int i = 0; i < SessionData.k_Players.Where(x => x != null).ToArray().Length; i++)
        {
            ...
            newBall.GetComponent<BallBeam>().SetColor(SessionData.k_Players[i].PlayerColor);
