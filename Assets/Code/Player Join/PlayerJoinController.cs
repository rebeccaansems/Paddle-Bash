...
public class PlayerJoinController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (readyPlayers > 1 && StartGameAnimator.GetBool("GameCanStart") == true)
        {
            ...
            {
                if (ReInput.players.GetPlayer(player.RewiredPlayerId).GetButtonDown("Enter") && !GameData.k_InputBlocked && levelSelectAnimator.GetBool("isOnLevelSelectScreen") == false)
                {
